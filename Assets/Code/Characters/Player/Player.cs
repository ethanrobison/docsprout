...
public class Player : MonoBehaviour {
	...
	void Update ()
	{
		...
		case PlayerState.walking:
			float x = Input.GetAxisRaw (buttons.moveX + Code.Game.Sesh.Input.GetButtonSuffix ());
			float y = Input.GetAxisRaw (buttons.moveY + Code.Game.Sesh.Input.GetButtonSuffix ());
			if (x * x + y * y < 0.01) {
		...
		float camX = Input.GetAxisRaw (buttons.camX + Code.Game.Sesh.Input.GetButtonSuffix ());
		float camY = Input.GetAxisRaw (buttons.camY + Code.Game.Sesh.Input.GetButtonSuffix ());
